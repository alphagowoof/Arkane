...
// Sentry mode off
	...
	client.on('message', message => {
		if (message.content.startsWith(`${prefix}${DisableIncomingMailCommand}`)){
			...
			message.channel.send('The Mod Mail function of Focus is now disabled.')
	...
	//Allow Incoming
		...
		if (message.content.startsWith(`${prefix}${EnableIncomingMailCommand}`)){
			...
			message.channel.send('The Mod Mail function of Focus is now enabled.')
	...
	//Help command (<prefix>mailhelp)
	client.on('message', message => {
		if (message.author.bot)return;
		if (!message.content.startsWith(`${prefix}mailhelp`))return;
		if (message.channel.type == 'dm')return;
		if(RequirePermissons == true){
		if (message.member.roles.cache.some(role => role.id === `${StaffRoleID}`)){}else{message.reply(NoPermReply);return;}}
		var helpmenu = []
